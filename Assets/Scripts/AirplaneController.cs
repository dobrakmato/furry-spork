...
public class AirplaneController : MonoBehaviour
{
    ...
    public bool shouldAscendBecauseOfVr;

    void Update()
    {
        isAscending = isAscendingBecauseOfStart | Input.GetButton("Fire1") | shouldAscendBecauseOfVr;
        if (isDead)
        {
            if (Score > 0)
            {
    ...
    private void FixedUpdate()
    {
        if (!isDead)
        {
            ...
            var horizontalInput = Mathf.Clamp(this.horizontalInput, -1f, 1f);
